...
print("Contenido extraído en /content/dataset:")
...
# Función para hacer múltiples predicciones
def analizar_multiples_imagenes():
    """
    Función para analizar múltiples imágenes de una vez
    """
    from google.colab import files
    print("📤 Selecciona múltiples imágenes MRI para analizar:")
    uploaded = files.upload()
...
# Función para probar con imágenes del dataset de prueba
def probar_con_dataset_prueba(num_muestras=5):
    """
    Función para probar el modelo con imágenes aleatorias del conjunto de prueba
    """
    ...
    for i, idx in enumerate(indices):
        ...
        # Color del borde según acierto/error
        color = 'green' if etiqueta_real == etiqueta_pred else 'red'

        plt.subplot(num_muestras, 1, i+1)
        plt.imshow(img)
        plt.title(f'Real: {resultado_real} | Predicción: {resultado_pred} (Confianza: {max(probabilidad, 1-probabilidad)*100:.1f}%)')
        plt.axis('off')
...
print("1. predecir_tumor(model) - Sube una imagen y obtén predicción")
print("2. analizar_multiples_imagenes() - Analiza varias imágenes a la vez")
print("3. probar_con_dataset_prueba() - Prueba con imágenes del dataset")
print("\n💡 Ejemplo de uso:")
...
# Para probar con 5 imágenes del dataset
...
!ls -la  # Ver qué archivos tienes disponibles
